...
class RelationController extends Controller
{
    ...
    /**
     * @api {get} /msk/relation Request all relations
     * @apiGroup Msk Relation
     * 
     ...
     *            }
     *        ],
     *        "first_page_url": "https://api.egundelik.com/api/msk/relation?page=1",
     *        "from": 1,
     *        "last_page": 1,
     *        "last_page_url": "https://api.egundelik.com/api/msk/relation?page=1",
     *        "next_page_url": null,
     *        "path": "https://api.egundelik.com/api/msk/relation",
     *        "per_page": 10,
     *        "prev_page_url": null,
     *        "to": 1,
     *        "total": 1
    ...
    public function index()
    {
        ...
        $relations = Relation::when(request('name'), function($q){
            $q->where('name','like','%'.request('name').'%');
        })->paginate(10);
        return $this->sendSuccess($relations);
    }
    /**
     * @api {post} /msk/relation Create relation
     * @apiGroup Msk Relation
     * 
     * @apiHeaderExample {json} Header-Example:
     * {
    ...
    {
        $validator = validator(request()->all(), [
            'name' => 'required|string|unique:relations'
        ]);
        if ($validator->fails()) {
        ...
        $relation->save();
        $relation->createLog();
        return $this->sendSuccess(['inserted_id' => $relation->id]);
    }
    /**
     * @api {delete} /msk/relation/:id Delete relation
     * @apiGroup Msk Relation
     * 
     * @apiHeaderExample {json} Header-Example:
     * {
    ...
    public function destroy($id)
    {
        ...
        $relation->createLog('deleted');
        $relation->delete();
        return $this->sendSuccess();
    }
    /**
     * @api {put} /msk/relation/:id Update relation
     * @apiGroup Msk Relation
     * 
     * @apiHeaderExample {json} Header-Example:
     * {
    ...
    {
        $validator = validator(request()->all(), [
            'name' => 'required|string|unique:relations,name,' . $id
        ]);
        if ($validator->fails()) {
            return $this->sendError($validator->errors()->toArray());
        }
        $relation = Relation::findOrFail($id);
        $relation->createLog('updated');
        $relation->name = request('name');
        $relation->save();
        return $this->sendSuccess();
    }
